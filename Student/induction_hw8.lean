...
def list_step {α : Type} : (head : α) → (tail : List α) → (len_tail : Nat) → Nat
...
def tree_empty_count := 0

-- 2. define step function
def tree_node_count : (n : Nat) → (l r : Tree Nat) → Nat → Nat → Nat
| _, _, _, count_l, count_r => 1 + count_l + count_r

-- 3. compose them using Tree.rec and apply to a_tree, expecting 3
#reduce (Tree.rec tree_empty_count tree_node_count : Tree Nat → Nat) a_tree
